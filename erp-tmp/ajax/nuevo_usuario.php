    ...
    require_once("../libraries/password_compatibility_library.php");
		...
		if (empty($_POST['firstname'])){
			...
			require_once ("../config/conexion.php");//Contiene funcion que conecta a la base de datos
				...
				//II parte
				$user_comunidad=$_POST['user_comunidad'];
				$user_hotspot=$_POST['user_hotspot'];
				$user_face=$_POST['user_face'];
				$user_whatsap=$_POST['user_whatsap'];
				$user_coment=$_POST['user_coment'];
				$user_porcent=$_POST['user_porcent'];
			
				//III parte
				$prec_hra1=intval($_POST['prec_hra1']);
				$prec_hra2=intval($_POST['prec_hra2']);
				$prec_hra3=intval($_POST['prec_hra3']);
				$prec_hra4=intval($_POST['prec_hra4']);
				$prec_hra5=intval($_POST['prec_hra5']);
				$prec_hra6=intval($_POST['prec_hra6']);
				$print_prec_fichas=intval($_POST['print_prec_fichas']);
                ...
                if ($query_check_user == 1) {
                    ...
                    $sql = "INSERT INTO users (firstname, lastname, user_name, user_password_hash, user_email, date_added,user_comunidad,user_hotspot,user_facebook,user_whatsap,
							user_comentarios,user_porcent,prec_hra1,prec_hra2,prec_hra3,prec_hra4,prec_hra5,prec_hra6,print_prec_fichas) 
                            VALUES('".$firstname."','".$lastname."','" . $user_name . "', '" . $user_password_hash . "', '" . $user_email . "','".$date_added.
							"', '" . $user_comunidad . "', '" . $user_hotspot . "', '" . $user_face . "', '" . $user_whatsap . "', '" . $user_coment . "', '" . $user_porcent . 
							"', '" . $prec_hra1 . "', '" . $prec_hra2 . "', '" . $prec_hra3 . "', '" . $prec_hra4 . "', '" . $prec_hra5 . "', '" . $prec_hra6 . "', '" . 
							$print_prec_fichas . "');";
					$query_new_user_insert = mysqli_query($con,$sql);

                    // if user has been added successfully
                    if ($query_new_user_insert) {
                        $sql_last=mysqli_query($con, "select LAST_INSERT_ID(user_id) as last from users order by user_id desc limit 0,1 ");
						$rw=mysqli_fetch_array($sql_last);
						//Se recupera el numero de factura insertado
						$id_last_user=$rw['last'];
                    } else {
                        $errors[] = "Lo sentimos , el last_insert fallo falló. Por favor, regrese y vuelva a intentarlo. Add User";
                    }
					
					// Parte II
					$sql_fh = "INSERT INTO fichas_horas VALUES(default,'$id_last_user', 0,0,0,0,0,0);";
					$query_insert_fh = mysqli_query($con,$sql_fh);
					// if user has been added successfully
                    if ($query_insert_fh) {

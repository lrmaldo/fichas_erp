<?php
	...
	$num_fact=trim(mysqli_real_escape_string($con,(strip_tags($_POST["num_fact"],ENT_QUOTES))));
	$pedido=trim(mysqli_real_escape_string($con,(strip_tags($_POST["num_pedido"],ENT_QUOTES))));
	...
	//Paso I Guardar encabezado insert compras--------------------------------------------------------------------------------------
	...
	while ($row=mysqli_fetch_array($sql_deta)) 	{
		...
		//Paso II insert en la tabla deta_compras
		$insert_detail = mysqli_query($con, "INSERT INTO deta_compras (id_compra, numero_factura, id_producto, cantidad, precio_costo,
		...
		//Paso III registro al inventario de la cantida inicial al registro del nuevo producto
		...
		//Paso IV proceso afectar la existencia real actual del producto
		...
		//Paso V Afectar el precio costo del catalogo de products con el nuevo pero almacenar el costo anterior,
	...
	//Paso VI actualizar el detalle detalle de compras
	$qry4 = "UPDATE compras set total_factura = '$total_factura' where id_compra = '$id_last_compra' ";
	$qry_update_total=mysqli_multi_query($con, $qry4);
	if ($qry_update_total) {
			//$messages[] = "Inventario ha sido ingresado satisfactoriamente.";
	} else {
			$errors []= "Lo siento algo ha salido mal intenta nuevamente. Ref: Compras, update total ".mysqli_error($con);
	}

	...
	$( "#Frm_theme_coment" ).submit(function( event ) {
		  $('#save_theme_coment').attr("disabled", true);
		 
		 var parametros = $(this).serialize();
			 $.ajax({
					type: "POST",
					url: "ajax/nuevo_theme_coment.php",
					data: parametros,
					beforeSend: function(objeto){
						$("#resultados_ajax").html("Mensaje: Cargando...");
					  },
					success: function(datos){
						$('#save_theme_coment').attr("disabled", false);
						//load(1);
						$('#NuevoThemComent').modal('hide');
					}
			});
		  event.preventDefault();
	})
